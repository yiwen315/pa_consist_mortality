...
data pooled;
  set pooled;
  if cigg in (1,2) then smkever=0; /* never smoker or past smoker >= 10y */
  else if cigg in (3,4) then smkever=1; /* current or past smoker < 10y */
run;
...
%pre_pm(data=never, out=never_py, timevar=periodnew,
...
%pm(data=never_py, case= alldead cvddead noncvddead , 
  exposure=joint , ref=1);


%pre_pm(data=ever, out=ever_py, timevar=periodnew,
        irt= irt88 irt90 irt92 irt94 irt96 irt98 irt00 irt02 irt04 irt06 irt08 irt10 irt12 irt14 irt16 irt18 irt20, 
        cutoff=cutoff, dtdx=dtdth, dtdth=dtdth,
        case= alldead cvddead noncvddead , 
        var=joint );

%pm(data=ever_py, case= alldead cvddead noncvddead , 
  exposure=joint , ref=1);




%let cov1 = white mifh canfh &alcog_ &qccalor_        &qcahei_ &pmh_;
...
proc export data=&event._out
            outfile='/udd/hpyzh/pa_consist/pooled/subgroup/pooled_smk.xlsx' dbms=xlsx replace;  sheet="&event.";
