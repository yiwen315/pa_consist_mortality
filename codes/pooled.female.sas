...
proc means data=female nmiss min max median mean; 
...
%pre_pm(data=female, out=female_py, timevar=periodnew,
...
%pm(data=female_py, case= alldead cvddead noncvddead, 
...
%mphreg9(data=female, outdat=&event._out,
         event=&event, 
         time=&time, 
...
proc export data=&event._out
            outfile='/udd/hpyzh/pa_consist/pooled/subgroup/pooled_female.xlsx' dbms=xlsx replace;  sheet="&event.";
