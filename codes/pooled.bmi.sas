...
data pooled;
     set pooled;
     if bmi<25 then bmihigh=0;
     else if bmi>=25 then bmihigh=1;
run;
...
proc freq data=bmilow;
  tables totalpacat2lag * pctmeetcat /missing;
run;


proc freq data=bmihigh;
  tables totalpacat2lag * pctmeetcat /missing;
run;



%pre_pm(data=bmilow, out=bmilow_py, timevar=periodnew,
...
%pm(data=bmilow_py, case= alldead cvddead noncvddead , 
  exposure=joint , ref=1);


%pre_pm(data=bmihigh, out=bmihigh_py, timevar=periodnew,
        irt= irt88 irt90 irt92 irt94 irt96 irt98 irt00 irt02 irt04 irt06 irt08 irt10 irt12 irt14 irt16 irt18 irt20, 
        cutoff=cutoff, dtdx=dtdth, dtdth=dtdth,
        case= alldead cvddead noncvddead , 
        var=joint );

%pm(data=bmihigh_py, case= alldead cvddead noncvddead , 
  exposure=joint , ref=1);
...
proc export data=&event._out
            outfile='/udd/hpyzh/pa_consist/pooled/subgroup/pooled_obe.xlsx' dbms=xlsx replace;  sheet="&event.";
