...
proc means data=pooled min median mean max;
     var cahei;
run;
...
data pooled;
     set pooled;
     if cahei<52.9 then diethigh=0;
     else if cahei>=52.9 then diethigh=1;
run;
...
proc freq data=dietlow;
  tables totalpacat2lag * pctmeetcat /missing;
run;


proc freq data=diethigh;
  tables totalpacat2lag * pctmeetcat /missing;
run;



%pre_pm(data=dietlow, out=dietlow_py, timevar=periodnew,
...
%pm(data=dietlow_py, case= alldead cvddead noncvddead , 
  exposure=joint , ref=1);


%pre_pm(data=diethigh, out=diethigh_py, timevar=periodnew,
        irt= irt88 irt90 irt92 irt94 irt96 irt98 irt00 irt02 irt04 irt06 irt08 irt10 irt12 irt14 irt16 irt18 irt20, 
        cutoff=cutoff, dtdx=dtdth, dtdth=dtdth,
        case= alldead cvddead noncvddead , 
        var=joint );

%pm(data=diethigh_py, case= alldead cvddead noncvddead , 
  exposure=joint , ref=1);




%let cov1 = white mifh canfh &alcog_ &qccalor_ &cigg_ &pkyrg_           &pmh_;
...
proc export data=&event._out
            outfile='/udd/hpyzh/pa_consist/pooled/subgroup/pooled_diet.xlsx' dbms=xlsx replace;  sheet="&event.";
