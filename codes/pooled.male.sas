...
proc means data=male nmiss min max median mean; 
...
%pre_pm(data=male, out=male_py, timevar=periodnew,
...
%pm(data=male_py, case= alldead cvddead noncvddead , 
...
%let cov1 = white mifh canfh &alcog_ &qccalor_ &cigg_ &pkyrg_ &qcahei_ ;



%macro consist_cox (event, time);

%mphreg9(data=male, outdat=&event._out,
         event=&event, 
         time=&time, 
         ...
         cutoff=cutoff, 
         dtdx=dtdth, dtdth=dtdth, id=id, agevar=agemo, tvar=periodnew, labels=F, strata=agemo periodnew ,
...
proc export data=&event._out
            outfile='/udd/hpyzh/pa_consist/pooled/subgroup/pooled_male.xlsx' dbms=xlsx replace;  sheet="&event.";
